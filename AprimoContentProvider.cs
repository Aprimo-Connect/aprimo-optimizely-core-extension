...
namespace Aprimo.Opti.Core
{
    public class AprimoContentProvider : ContentProvider
    {
        ...
        public AprimoContentProvider(
            ...
            IAprimoFieldPropertyResolver aprimoFieldPropertyResolver,
        ...
        protected override IContent LoadContent(ContentReference contentLink, ILanguageSelector languageSelector)
        {
            ...
            try
            {
                ...
                var file = aprimoAssetService.GetAsset(masterId);
                string extension = string.Empty;
                if (file != null)
                {
                    ContentType contentType = contentTypeRepository.Load(typeof(AprimoGenericFile));
                    if (file?.Embedded?.MasterFileVersion?.FileExtension != null)
                    {
                        extension = file?.Embedded?.MasterFileVersion?.FileExtension;
                        var modelType = aprimoContentAssetResolver.GetFirstMatching(extension);
                    ...
                    {
                        Parent = contentLoader.Get<ContentFolder>(EntryPoint)
                    }) as AprimoAssetData;
                    ...
                    {
                        var fieldProperties = aprimoFieldPropertyResolver.Resolve(typeof(AprimoAssetData));
                    ...
                    if (assetData is IAprimoImage aprimoImageAsset)
                    {
                        ...
                        var asset = aprimoAssetPersistantService.GetAsset(Guid.Parse(masterId), renditionId);
                        if (asset != null)
                        {
                            ...
                            // populate thumbnails
                            var properties = aprimoTransformDescriptorPropertyResolver.Resolve(contentType.ModelType);
            ...
            {
                log.Error("Error locating Aprimo asset", ex);
            }
        ...
        protected override IList<GetChildrenReferenceResult> LoadChildrenReferencesAndTypes(ContentReference contentLink, string languageID, out bool languageSpecific)
        {
            ...
            {
                var allAssets = identityMappingService.List(AprimoConstants.ProviderKey);
                foreach (var asset in allAssets)
                {
                    ...
                    try
                    {
                        var persistantAsset = aprimoAssetPersistantService.GetAsset(asset.ContentLink);
                        if (persistantAsset != null)
                        {
                            if (!string.IsNullOrWhiteSpace(persistantAsset.Extension))
                            {
                                childAsset.ModelType = aprimoContentAssetResolver.GetFirstMatching(persistantAsset.Extension);
                            }
                        }
                    }
                    catch (Exception)
        ...
        protected override ContentResolveResult ResolveContent(ContentReference contentLink)
        {
            ...
            if (contentLink.ProviderName != ProviderKey)
                return null;
        ...
        {
            var contentItem = identityMappingService.Get(contentGuid);
            if (contentItem == null)
                return null;
